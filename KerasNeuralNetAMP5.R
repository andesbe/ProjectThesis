...
input <- as.data.frame(rbind(input))
output <- as.data.frame(rbind(output))
...
model <-keras_model_sequential()%>%
  layer_dense(units = 175, activation = "relu", input_shape =c(4),  kernel_regularizer = regularizer_l1(l = 0.0001))%>%
  layer_dropout(0.35) %>%
...
history <- model%>%
  fit(x.train, y.train, epochs = 1000, batch_size = 120, validation_split = 0.2)
...
model_save <- keras_model(model, history)
